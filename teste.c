...
#define DURATION 0.3 //segundos
#define AMPLITUDE 12000

//Funçõa mapear vogais para notas (canal esquerdo)
...
//Descobrir o índice da nota atual
    ...
    switch (c) {
        ...
        case 'c': case 'k': case 's': case 'q':
            return escala[(idx + 1) % 7]; //próxima nota (segunda)
        case 'd': case 't':
...
//Lendo arquivo de texto (.txt)
    FILE* f = fopen("entrada.txt", "r"); //Definindo que teremos um arquivo chamado "entrada.txt" na pasta (que será lido pelo programa)
    if (!f) {
